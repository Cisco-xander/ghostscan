...
class ReportingModule:
    ...
    def _build_markdown_lines(self) -> list:
        ...
        lines += [
            ...
            f"| **Date**    | {now} |",
            f"| **Tool**    | GhostScan v3.0 — Kali Linux Framework |",
            f"",
            f"---",
            f"",
            f"## Executive Summary",
        ...
        lines += [
            "---",
            "",
            f"*Report generated by GhostScan v3.0 on {now}*",
    ...
    def _build_html(self) -> str:
        ...
        # HTML sections (computed before the template to avoid nested f-strings)
        port_section = "<div class='no-data'>No port scan results.</div>" if not port_rows else f"""
  <table class="data-table">
    <thead><tr><th>Host</th><th>Port</th><th>Service</th><th>Version</th></tr></thead>
    <tbody>{port_rows}</tbody>
  </table>"""
        sub_section = "<div class='no-data'>No subdomains found.</div>" if not sub_rows else f"""
  <table class="data-table">
    <thead><tr><th>Subdomain</th><th>IPs</th><th>Source</th></tr></thead>
    <tbody>{sub_rows}</tbody>
  </table>"""
        dir_section = "<div class='no-data'>No results.</div>" if not dir_rows else f"""
  <table class="data-table">
    <thead><tr><th>Status</th><th>Path</th><th>Size</th></tr></thead>
    <tbody>{dir_rows}</tbody>
  </table>"""
        js_section = "<div class='no-data'>No secrets detected.</div>" if not js_rows else f"""
  <table class="data-table">
...
Generated: {now} &nbsp;|&nbsp; GhostScan v3.0</div>
...
<footer>
  Generated by <strong>GhostScan v3.0</strong> — Kali Linux Penetration Testing Framework<br>
  <em>For authorized security assessments only. Unauthorized use is illegal.</em>
    ...
    def _write_pdf(self) -> str:
        ...
        story.append(Paragraph(f"<b>Framework:</b> GhostScan v3.0 — Kali Linux", styles["Normal"]))
